...
**Current reference state, 15 September 2026:** authenticated MCP and lexical Brain retrieval are ready, caller identity is verified, seven production domain agents are online and 45 of 55 core capabilities are ready. Four capabilities are degraded, four await configuration, one is blocked by a dependency and one is configured but unverified. A full offsite restore passed in isolation on 9 September with one inherited ledger warning. Automated company-source ingestion and the Context Compiler were degraded at the release boundary, broad unattended execution is not deployed, and several services still share infrastructure failure domains.
