...
Reports:
  - integrations connected (from /opt/compai/credentials/index.json)
...
AGENT_UNITS = [
    "compai-mcp",
    "compai-cs", "compai-finance", "compai-ops", "compai-marketing",
    "compai-merch", "compai-retail", "compai-hr",
    "compai-tunnel",
]
...
def _pretty(report: dict) -> None:
    print(f"\n{common.BOLD}Compai status — {report['brand']}{common.RESET}")
    ...
    if not report["integrations"]:
        print(f"  {common.DIM}none connected yet — run `compai-init connect <service>`{common.RESET}")
    else:
    ...
    if t:
        ...
        print(f"  {common.DIM}not configured — run `compai-init tunnel <subdomain>`{common.RESET}")
