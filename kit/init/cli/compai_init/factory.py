...
Commands:
  enable  --domain <cs>    Install factory templates into /opt/compai/agents/<domain>/
...
{common.BOLD}Next:{common.RESET}
  compai-init factory show --domain {domain}     # inspect sub-agents + contracts
...
def cmd_run_once(args, *, home: Path, brand: str):
    ...
    from compai_init.factory_runtime import config as fc_cfg, orchestrator, trace as trace_mod

    dst = _factory_dst(home, args.domain)
    if not dst.exists():
        common.err(f"factory '{args.domain}' is not enabled. Run: compai-init factory enable --domain {args.domain}")
        return
    ...
    if not args.mock_llm:
        ...
        try:
            from compai_init.llm import config as llm_config
            configured = llm_config.list_configured(home)
            if not configured:
                common.err("no LLM providers configured. Run: compai-init llm configure  (or use --mock-llm)")
                return
        except ImportError:
...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    sp = inner.add_parser("enable", help="Install a factory into /opt/compai/agents/<domain>/factory/")
