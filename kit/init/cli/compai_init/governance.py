...
def cmd_enable(args, *, home: Path, brand: str):
    ...
    for slug in META_AGENTS:
        src = sys_src / f"compai-{slug}.service.tmpl"
        unit_dst = Path(f"/etc/systemd/system/compai-{slug}.service")
        try:
...
{common.BOLD}Next:{common.RESET}
  systemctl enable --now compai-critic
  systemctl enable --now compai-guardrail
  systemctl enable --now compai-compliance
...
def cmd_status(args, *, home: Path, brand: str):
    ...
    for slug in META_AGENTS:
        state = _service_state(f"compai-{slug}")
...
def cmd_disable(args, *, home: Path, brand: str):
    ...
    for slug in META_AGENTS:
        try:
            subprocess.run(["systemctl", "disable", "--now", f"compai-{slug}"], check=False, timeout=10)
            common.ok(f"disabled compai-{slug}")
        except Exception as e:
            common.warn(f"could not disable compai-{slug}: {e}")
