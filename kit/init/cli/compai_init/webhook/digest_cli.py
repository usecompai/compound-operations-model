"""compai-init digest — Slack digest CLI."""
...
from compai_init.webhook import digest as dg
...
def cmd_configure(args, *, home: Path, brand: str):
    ...
    print(
        ...
        "\n  2. Add a new webhook, choose the channel (e.g. #compai-ops)"
...
def cmd_now(args, *, home: Path, brand: str):
    ...
    if not cfg.get("slack_webhook"):
        common.err("Slack webhook not configured. Run: compai-init digest configure")
        return
...
def cmd_schedule(args, *, home: Path, brand: str):
    cfg = dg.load_config(home)
    if not cfg.get("slack_webhook"):
        common.err("configure a Slack webhook first: compai-init digest configure")
        return
...
def cmd_status(args, *, home: Path, brand: str):
    ...
    if cfg.get("schedule_cron"):
        ...
        print(f"  Scheduled: {common.DIM}no cron installed — run: compai-init digest schedule{common.RESET}")
    print()
