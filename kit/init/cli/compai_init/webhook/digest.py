...
Cron-based, not daemon-based. Install via `compai-init digest schedule`
which appends to the compai user's crontab (08:00 UTC daily).
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Config — /opt/compai/credentials/digest.json
# ─────────────────────────────────────────────────────────────────────────────
...
def install_cron(home: Path, hour_utc: int = 8, minute: int = 0) -> tuple[bool, str]:
    """Append a daily cron line for the compai user. Idempotent."""
    line = f"{minute} {hour_utc} * * * /usr/local/bin/compai-init digest now >> {home}/logs/digest.log 2>&1"
    try:
        existing = subprocess.run(
            ["sudo", "-u", "compai", "crontab", "-l"],
