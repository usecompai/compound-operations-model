...
Automatic behavior:
  ...
  - Records each call in the usage DB (compai_init.llm.usage)
...
from compai_init.llm.providers import anthropic as prov_anthropic
from compai_init.llm.providers import openai as prov_openai
from compai_init.llm.providers import gemini as prov_gemini
from compai_init.llm.providers import qwen as prov_qwen
from compai_init.llm.providers import minimax as prov_minimax
...
def _home() -> Path:
    return Path(os.environ.get("COMPAI_HOME", "/opt/compai"))
...
def _resolve_call_plan(
    ...
    if not configured:
        raise NoProvidersConfiguredError(
            "No LLM providers configured. Run: compai-init llm configure"
        )
    ...
    if not plan:
        raise NoProvidersConfiguredError(
            "No provider in the call plan is configured. Run: compai-init llm configure"
        )
    return plan
