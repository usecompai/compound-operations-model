#!/usr/bin/env python3
"""
compai-init — CLI dispatcher for Compai brand bootstrap operations.

Installed at /usr/local/bin/compai-init by install.sh.
...
All subcommands respect $COMPAI_HOME (default /opt/compai).
"""
...
from compai_init import connect, tunnel, team_join, status, key as key_cmd, assess as assess_cmd, governance as gov_cmd, factory as factory_cmd  # noqa: E402
from compai_init.llm import cli as llm_cli  # noqa: E402
from compai_init import event as event_cli  # noqa: E402
from compai_init.webhook import cli as webhook_cli  # noqa: E402
from compai_init.webhook import digest_cli  # noqa: E402
from compai_init import onboarding as onboarding_cli  # noqa: E402
from compai_init import setup_brand as setup_brand_cli  # noqa: E402
from compai_init.ingest import cli as ingest_cli  # noqa: E402
...
def _home() -> Path:
    p = Path(os.environ.get("COMPAI_HOME", "/opt/compai"))
    if not p.exists():
...
def _brand(home: Path) -> str:
    """Resolve brand slug from /opt/compai/brain/knowledge/ layout."""
...
def cmd_distil(args):
    print(f"{GOLD}{BOLD}compai-init distil{RESET} · v0.5 (not yet implemented)")
    print("This will auto-generate 6 per-area contexts after 30d of ingested data.")
    print("Tracking: knowledge/projects/compai/brand-bootstrap-v0.1.md")
    sys.exit(2)
...
def main():
    ap = argparse.ArgumentParser(prog="compai-init", description=__doc__)
